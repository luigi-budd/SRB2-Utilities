...
addHook("HUD", function(v,p,cam)
	if not cv_esp.value then return end
	local me = p.realmo
	...
	for play in players.iterate()
		...
		*/
		...
		if gametyperules & GTR_TAG
			local red = (play.pflags & PF_TAGIT and true or false)
			if (p.pflags & PF_TAGIT) then red = not $; end
			color = red and SKINCOLOR_RED or SKINCOLOR_GREEN
		elseif gametype == GT_LTMMURDERMYSTERY
			local role = play.mm.role
			if role == ROLE_MURDERER
				color = SKINCOLOR_RED
			elseif role == ROLE_SHERIFF
				color = SKINCOLOR_BLUE
			elseif role == ROLE_CIVILIAN
	...
	for k,item in pairs(drawwork)
		...
		local w2s = K_GetScreenCoords(v, p, cam, play.mo)
